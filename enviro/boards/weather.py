...
def log_rain():
  ...
  if len(rain_entries) > 190:
    logging.warn(">>> * * Rain log file exceeded 190 entries and was truncated * *")
...
def wind_speed(sample_time_ms=1000):
  ...
  while time.ticks_diff(time.ticks_ms(), start) <= sample_time_ms:
    windtuple = (wind_speed_pin.value(),time.ticks_ms())
    if windtuple[0] != state and windtuple[1] != ticks[-1] if len(ticks) != 0 else start: # sensor output changed
      # record the time of the change and update the state
      ticks.append(windtuple[1])
      state = windtuple[0]

  logging.debug(f">>> ? ? wind speed stats - start:{start}, ticks:{ticks} ? ?") 
  ...
  logging.debug(f">>> ? ? wind speed stats - speed: {wind_m_s} ? ?") 
...
def wind_direction():
  ...
  logging.debug(f">>> ? ? wind direction - {offset_wind_direction} ? ?") 
...
def get_sensor_readings(seconds_since_last, is_usb_power):
  ...
  if ltr_data is not None:
      data["luminance"] = round(ltr_data[BreakoutLTR559.LUX], 2)
